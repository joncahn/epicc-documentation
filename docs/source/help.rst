...
The first column `GID` must match the `GID` from the first column of the `gaf_file` above. 
...
An replace the `GOdatabase` entry in the config file for the corresponding species:: 
